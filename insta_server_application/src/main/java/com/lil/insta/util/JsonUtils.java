...
public class JsonUtils {
    ...
    /**
     * Returns the JsonObject at the given index from a JsonArray.
     * 
     * Checks that the index exists and that the value at that index is a JsonObject.
     * 
     * @param jsonArray The JsonArray to fetch from
     * @param index The index of the desired JsonObject
     * @return The JsonObject at the given index
     * @throws MissingValueException If the index is out of range or not a JsonObject
     */
    public static JsonObject getJsonObjectAtIndex(JsonArray jsonArray, int index) throws MissingValueException {
        if (index < 0 || index >= jsonArray.size()) {
            throw new MissingValueException(format("JsonArray index %d is out of range. Size is %d.", index, jsonArray.size()));
        }

        JsonValue value = jsonArray.get(index);
        if (value.getValueType() != JsonValue.ValueType.OBJECT) {
            throw new MissingValueException(format("Value at index %d is not a JsonObject (found %s).", index, value.getValueType()));
        }
     ...
     * @return
     */
    public static JsonObject stringToJsonObject(String jsonObjectStr) {
        // The 'try resouce' block ensures that reader.close() and jsonReader.close() get called.
        try (StringReader reader = new StringReader(jsonObjectStr); JsonReader jsonReader = Json.createReader(reader)) {
     ...
     */
    public static JsonValue stringToJsonValue(String jsonValueStr) {
        try (StringReader reader = new StringReader(jsonValueStr); JsonReader jsonReader = Json.createReader(reader)) {
     ...
     * @return A JsonArray
     */
    public static JsonArray stringToJsonArray(String jsonArrayStr) {
        try (StringReader reader = new StringReader(jsonArrayStr); JsonReader jsonReader = Json.createReader(reader)) {
